...
module led_holder()
{
	...
	difference()
	{
		union() {
			color("yellow") {
				translate([(length-c_width)/2, 0, thick/2]) cube([length, width, thick], center=true);
				translate([0, 0, c_width/2]) cube([c_width, width, c_width], center=true);
			}
			
		}
		translate([0, 0, 9.5]) rotate([0, 45, 0])  cube([c_width, width+2, c_width], center=true);
		translate([0, 0, 11]) rotate([0, 0, 0])  cube([c_width+2, width+2, c_width], center=true);
		translate([0, 0, c_width/2]) rotate([90, 0, 0]) cylinder(d=4.7, h=width+2, center=true);
		translate([(length-c_width)/2, 0, -0.1]) rotate([0, 0, 0]) cylinder(d=3, h=20, center=true);
	}
}
